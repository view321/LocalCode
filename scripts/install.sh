...
setup_llama() {
  if [[ "${LOCALCODE_SKIP_LLAMA:-}" == "1" ]]; then
    warn "Skipping llama-server setup (LOCALCODE_SKIP_LLAMA=1)"
    return 0
  fi
  ...
  info "Installing llama-server (required for local deploys and the Bonsai assistant)"
  if "$bin" setup; then
