...
4. When the user asks to deploy/run a model, check the card with hf.model_card, then call deploy_model directly — do not tell the user to do it manually. OMIT the backend argument so it auto-selects the right one for the weight format (GGUF→llama.cpp/Ollama, safetensors/AWQ/GPTQ→vLLM/SGLang, colibrì int4 containers→colibri/colibri-hy3) and auto-fills a fitting context and the card's flags; set backend only to deliberately override. Never deploy a GGUF model on vLLM or SGLang — those load safetensors/AWQ/GPTQ, not GGUF. Pre-converted colibrì containers (out-*.safetensors shards, e.g. GLM-5.2-colibri-int4 or Hy3-colibri-int4 repos) run ONLY on the colibrì engines: colibri serves GLM-5.2, colibri-hy3 serves Tencent Hy3 (and GLM-5.2). They stream experts from disk, so huge containers run in modest RAM but need the full repo on local disk.
